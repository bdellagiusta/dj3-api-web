<template>
  <div>
          ...
          <template #default="{ row }">
            <template v-if="!row.is_file">
              <el-table size="small" :data="row.info?.files?.filter((v, k) => k < showDirFileNum)" fit>
                <el-table-column prop="0" :label="T('FileName')" align="center" width="150"
              ...
              <el-button size="small" v-if="row.info.files.length > showDirFileNum" style="width: 100%;margin-top: 5px"
                type="primary" @click="showAllFile(row.info.files)">
    ...
    <el-dialog v-model="allFilesVisible" :title="T('File')">
      ...
      <el-button @click="allFilesVisible = false" style="margin-top: 20px;width: 100%" type="primary">{{ T('Close')
        }}</el-button>
    </el-dialog>
  </div>
</template>
