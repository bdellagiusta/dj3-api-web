  ...
  <el-container class="address-book-con">
    <el-aside width="200px" class="aside">
      ...
      <div class="aside-tags">
        ...
        <div v-for="t in tagListRes.list" :key="t.id" class="tag" :class="{ checked: checkedTags.includes(t.name) }"
          :style="{ backgroundColor: checkedTags.includes(t.name) ? t.color : '' }" @click="toggleTag(t)">
          <span class="dot" :style="{ background: checkedTags.includes(t.name) ? '#fff' : t.color }"></span> <span>{{ t.name
            }}</span>
        </div>
