        ...
        <el-table-column :label="T('Actions')" align="center" class-name="table-actions" width="600" fixed="right">
          <template #default="{row}">
            <template v-if="row.id>0">
              <!-- Botón para mostrar reglas de compartición -->
...
// Hook personalizado para manejar la lista de direcciones
const {
  listRes,
  listQuery,
  getList,
...
// Obtener lista al montar el componente
onMounted(getList)

// Actualizar lista al cambiar página
watch(() => listQuery.page, getList)

// Actualizar lista al cambiar tamaño de página
watch(() => listQuery.page_size, handlerQuery)

// Comprobar si el usuario es "hiperdino"
const isHiperdino = computed(() => userInfo.value?.name === 'hiperdino')

// Construir la lista, agregando un registro especial en la primera página
...
// Para almacenar la fila seleccionada y mostrar reglas
const clickRow = ref({})
